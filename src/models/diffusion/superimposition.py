...
def weighted_rigid_align(x, x_gt, w):
    ...
    mu_gt = (w.unsqueeze(-1) * x_gt).sum(dim=1) / w.sum(dim=1, keepdim=True)  # shape (b, 3)
    ...
    cov_matrix = torch.einsum('bl,bli,blj->bij', w, x_gt_centered, x_centered)  # shape (b, 3, 3)
    ...
    det_R = torch.det(R)
    F = torch.diag(torch.tensor([1, 1, -1], dtype=x.dtype, device=x.device)).unsqueeze(0)  # shape (1, 3, 3)

    R = torch.where(det_R.unsqueeze(-1).unsqueeze(-1) < 0, torch.matmul(U, torch.matmul(F, V)), R)
