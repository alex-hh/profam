...
class BasePreprocessor:
    ...
    def _preprocess_protein_data(
        ...
        if max_tokens is not None:
            assert (
                tokenized.data["input_ids"].shape[-1] <= max_tokens
            ), f"{tokenized.data['input_ids'].shape[-1]} > {max_tokens}"
...
class ParquetStructurePreprocessor(BasePreprocessor):
    def __init__(
        ...
        if interleave_proteins:
            ...
            interleaved_transform_fns.append(
                transforms.concatenate_interleaved_proteins
            )
