...
from lightning.pytorch.utilities.signature_utils import is_overridden
...
class StepGradientAccumulationScheduler(Callback):
    ...
    def __init__(self, scheduling: Dict[int, int]):
        ...
        if not isinstance(scheduling, dict) or not scheduling:
            raise MisconfigurationException("`scheduling` must be a non-empty dictionary.")
        ...
        if any(not isinstance(factor, int) or factor < 1 for factor in scheduling.values()):
            raise MisconfigurationException(
        ...
        if not self.sorted_steps:
             raise MisconfigurationException("`scheduling` must define at least one step, typically step 0.")

    def _get_accumulate_grad_batches(self, current_global_step: int) -> int:
        accumulate_grad_batches = 1  # Default, should be overridden by self.scheduling[0]
    ...
    def on_train_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        if (overridden_optimizer_step or overridden_optimizer_zero_grad) and is_any_factor_greater_than_one:
            rank_zero_warn(
    ...
    def on_train_batch_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", batch: Any, batch_idx: int) -> None:
        trainer.accumulate_grad_batches = self._get_accumulate_grad_batches(trainer.global_step)
