...
class MInterface(MInterface_base):
    ...
    def forward(self, batch, batch_idx, mode="train", level=8):
        ...
        X_pred, all_preds, vq_loss, vq_count, vq_code = self.model(
            X,
            chain_encoding,
            edge_idx,
            batch_id,
    ...
    def batch_data_single_chain(self, list_of_data):
        ...
        edge_idx = knn_graph(
            X_all[:, 1], k=30, batch=batch_id_all, loop=True, flow="target_to_source"
        )
        return {
            "title": name_all,
            "S": S_all.cuda(),
            ...
            "batch_id": batch_id_all.cuda(),
            "chain_encoding": chain_encoding_all.cuda(),
            "temp": 1e-8,
            "mode": "eval",
        }
    ...
    def encode_only(self, batch, level=8):
        ...
        h_V, vq_code, vq_loss = self.model.vq(
            h_V, temperature=temp, mode="eval", vqshortcut=False, level=level
        )
        vq_code_all = []
    ...
    def encode_protein(self, protein_init, level=8, Ablation=False):
        ...
        _, vq_code, vq_loss = self.model.encoding(
            X,
            chain_encoding,
            edge_idx,
            batch_id,
