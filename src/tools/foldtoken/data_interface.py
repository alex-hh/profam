...
class MyDataLoader(DataLoader):
    def __init__(
        self, data_module, dataset, num_workers=8, virtual_frame_num=0, *args, **kwargs
    ):
        super(MyDataLoader, self).__init__(
            dataset, num_workers=num_workers, *args, **kwargs
        )
...
def memory_efficient_collate_fn(batch):
    ...
    for key in batch[0].keys():
        if key in ["edge_idx"]:
            ret[key] = torch.cat(
                [one[key] + shift[idx] for idx, one in enumerate(batch)], dim=1
            )
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            self.trainset = self.instancialize(
                split="train", data=self.dataset.all_data["train"]
            )

            self.valset = self.instancialize(
                split="val", data=self.dataset.all_data["val"]
            )

        if stage == "test" or stage is None:
            self.testset = self.instancialize(
                split="test", data=self.dataset.all_data["test"]
            )
