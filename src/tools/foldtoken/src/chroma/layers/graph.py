...
from src.chroma.layers.attention import Attention
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if mask_i is not None:
                node_h = node_h * (mask_i.unsqueeze(-1) != 0).type(node_h.dtype)
            if mask_ij is not None:
                edge_h = edge_h * (mask_ij.unsqueeze(-1) != 0).type(node_h.dtype)
...
class GraphLayer(nn.Module):
    ...
    def forward(
        ...
        if mask_i is not None:
            mask_i = (mask_i != 0).type(node_h.dtype)
        if mask_ij is not None:
            mask_ij = (mask_ij != 0).type(node_h.dtype)
