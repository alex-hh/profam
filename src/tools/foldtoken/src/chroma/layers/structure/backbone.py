...
class RigidTransformer(nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            ...
            # )
            X_mean = stable_masked_mean(X_flat, mask_flat, dim=1, keepdim=True)
        else:
...
class FrameBuilder(nn.Module):
    ...
    def _build_O(self, X_chain: torch.Tensor, C: torch.LongTensor):
        ...
        mask = (C > 0).type(C.dtype).reshape(list(C.shape) + [1, 1])
    ...
    def inverse(
        ...
        mask = (C > 0).type(X.dtype).unsqueeze(-1)
...
class GraphBackboneUpdate(nn.Module):
    ...
    def _update_neighbor_global_affine_transform(
        ...
        #             't_global_i': t_gi,
        #             'logit_global_i': logit_gi
        #             }, '/huyuqi/gaozhangyang/ProteinStructBench/data.pt')
        X_update = self.frame_builder(R_i, t_i, C)
        return X_update, R_ji, t_ji, logit_ji
    ...
    def forward(
        ...
        if self.unconstrained:
            ...
            R_i = R_i.unsqueeze(-3)
            t_i = torch.zeros_like(t_i, dtype=X.dtype).unsqueeze(-2)
...
class LossBackboneResidueDistance(nn.Module):
    ...
    def forward(
        ...
        # D_error = stable_masked_mean(D_error, mask_2D, dim=-1)
        return D_error
...
def center_X(X: torch.Tensor, C: torch.LongTensor) -> torch.Tensor:
    ...
    """
    mask_expand = (
        (C > 0).to(X.dtype).reshape(list(C.shape) + [1, 1]).expand([-1, -1, 4, -1])
    )
...
def atomic_mean(
    ...
    # )
    X_mean = stable_masked_mean(X_flat, mask_atomic, dim=1, keepdim=True)
...
def expand_chain_map(C: torch.LongTensor) -> torch.Tensor:
    ...
    mask_C = (idx_expand == C_expand).type(C.dtype)
    return mask_C
