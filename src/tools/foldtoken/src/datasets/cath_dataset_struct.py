...
class CATHDataset(data.Dataset):
    ...
    def _get_features(self, batch):
        ...
        def node_mask_select(x):
            ...
            out = torch.masked_select(x, mask_bool.unsqueeze(-1)).reshape(
                -1, x.shape[-1]
            )
            out = out.reshape(-1, *shape[2:])
            return out

        batch_id = torch.arange(mask_bool.shape[0], device=mask_bool.device)[
            :, None
        ].expand_as(mask_bool)
        ...
        C_a = X[:, 1, :]
        edge_idx = knn_graph(
            C_a, k=self.k_neighbors, batch=batch_id, loop=True, flow="target_to_source"
        )
        batch = {
            "title": batch["title"],
            "X": X,
            ...
            "batch_id": batch_id,
            "mask": mask,
            "num_nodes": torch.tensor(X.shape[0]).reshape(
                1,
            ),
    ...
    def sv2pdb(self, item):
        ...
        name = item["title"]
        protein.to_PDB(
            f"/huyuqi/xmyu/VQProteinFormer/step1_VQDiff/pdbs/cath4.3/{name}.pdb"
        )
    ...
    def load_cath_data(self, index):
        ...
        if L > self.max_length:
            ...
            item["chain_mask"] = item["chain_mask"][
                truncate_index : truncate_index + self.max_length
            ]
            item["chain_encoding"] = item["chain_encoding"][
                truncate_index : truncate_index + self.max_length
            ]
        item["X"] = torch.from_numpy(
            np.stack([item["N"], item["CA"], item["C"], item["O"]], axis=1)
        ).float()
