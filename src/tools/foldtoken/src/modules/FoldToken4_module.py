...
from src.tools.affine_utils import (
    Rigid,
    Rotation,
    invert_rot_mat,
    quat_to_rot,
...
def build_MLP(
    n_layers, dim_in, dim_hid, dim_out, dropout=0.0, activation=nn.ReLU, normalize=True
):
    if normalize:
        layers = [
        ...
        layers = [nn.Linear(dim_in, dim_hid), nn.Dropout(dropout), activation()]
...
class GeoFeat(nn.Module):
    def __init__(self, geo_layer, num_hidden, virtual_atom_num, dropout=0.0):
        ...
        self.we_condition = build_MLP(
            geo_layer,
            4 * virtual_atom_num * 3 + 9 + 16 + 32,
            num_hidden,
            num_hidden,
            dropout,
        )
    ...
    def forward(self, h_V, h_E, T_ts, edge_idx):
        ...
        RKs = torch.einsum(
            "eij,enj->eni",
            T_ts._rots._rot_mats[:, 0],
            V_local[src_idx].view(num_edge, -1, 3),
        )
...
class PiFoldAttn(nn.Module):
    def __init__(self, attn_layer, num_hidden, num_V, num_E, dropout=0.0):
        ...
        self.__dict__.update(locals())
        self.num_heads = 4
        self.W_V = nn.Sequential(nn.Linear(num_E, num_hidden), nn.GELU())
    ...
    def forward(self, h_V, h_E, edge_idx):
        ...
        w = self.Bias(torch.cat([h_V[src_idx], h_E, h_V[dst_idx]], dim=-1)).view(
            E, n_heads, 1
        )
...
class GeoFeaturizer(nn.Module):
    ...
    def from_X_to_features(
        ...
        T = Rigid.make_transform_from_reference(
            X[:, 0].view(-1, 3), X[:, 1].view(-1, 3), X[:, 2].view(-1, 3)
        )
        T_ts = T[edge_idx[1], None].invert().compose(T[edge_idx[0], None])
        if virtual_frame_num > 0:
            V, E = self.get_interact_feats(
                T, T_ts, X, edge_idx, batch_id, atom_embed=atom_embed
            )
            V_g, T_g, T_gs, edge_idx_g, batch_id_g = self.construct_virtual_frame(
                T, batch_id, virtual_frame_num
            )
            V_g = self.int_embedding(V_g, V.shape[-1])
            E_g = torch.zeros(
                (edge_idx_g.shape[1], E.shape[1]), device=V.device, dtype=V.dtype
            )
            V, E, T, T_ts, batch_id, edge_idx = self.merge_local_global(
                V,
                V_g,
                E,
                E_g,
            ...
            chain_encoding = torch.cat(
                [chain_encoding, torch.ones_like(batch_id_g) + 1000], dim=0
            )
        else:
            V, E = self.get_interact_feats(
                T, T_ts, X, edge_idx, batch_id, atom_embed=atom_embed
    ...
    def construct_virtual_frame(self, T, batch_id, num_global=3):
        num_nodes = scatter_sum(
            torch.ones_like(batch_id), batch_id, dim_size=batch_id.max() + 1
        )

        global_src = torch.cat(
            [
                batch_id + batch_id.shape[0] + k * num_nodes.shape[0]
                for k in range(num_global)
            ]
        )
        global_dst = torch.arange(batch_id.shape[0], device=batch_id.device).repeat(
            num_global
        )
        ...
        batch_id_g = torch.arange(num_nodes.shape[0], device=batch_id.device).repeat(
            num_global
        )
        ...
        rot_gs = torch.eye(3, device=X_c.device)[None, None].repeat(
            edge_idx_g.shape[1], 1, 1, 1
        )
        trans_gs = torch.zeros(3, device=X_c.device)[None, None].repeat(
            edge_idx_g.shape[1], 1, 1
        )

        T_gs = Rigid(Rotation(rot_gs), trans_gs)

        h_V_g = torch.arange(num_global, device=batch_id.device).repeat(
            num_nodes.shape[0]
        )
    ...
    @classmethod
    @torch.no_grad()
    def get_interact_feats(
        self, T, T_ts, X, edge_idx, batch_id, num_rbf=16, atom_embed=None
    ):
        dtype = X.dtype
        device = X.device
        ...
        diffE = (
            T[src_idx, None].invert().apply(torch.cat([X[src_idx], X[dst_idx]], dim=1))
        )
...
class GeneralGNN(nn.Module):
    def __init__(
        ...
        self.attention = PiFoldAttn(
            attn_layer, num_hidden, num_hidden, num_hidden, dropout
        )
...
class StructureEncoder(nn.Module):
    ...
    def decouple_local_global(
        self, h_V, h_E, batch_id, edge_idx, h_V_g, h_E_g, batch_id_g
    ):
    ...
    def forward(
        ...
        E=None,
        T_ts=None,
        batch_id_extend=None,
        edge_idx_extend=None,
        virtual_frame_num=3,
        temperature=1.0,
    ):
        N = batch_id.shape[0]
        if V is None:  # for sampling, initial V, E, T_ts
            (
                ...
                edge_idx,
                chain_encoding,
            ) = GeoFeaturizer.from_X_to_features(
                X, edge_idx, batch_id, chain_encoding, virtual_frame_num
            )
        else:
            batch_id = batch_id_extend
            edge_idx = edge_idx_extend

        h_V, h_E = self.node_embedding(V), self.edge_embedding(
            E
        ) + self.chain_embedding(
            (chain_encoding[edge_idx[0]] == chain_encoding[edge_idx[1]]).long()
        )
...
class GeneralE3GNN(nn.Module):
    def __init__(self, gnn_layers, num_hidden, version="CATH-AE"):
        ...
        self.gnns = nn.ModuleList(
            [GeneralGNN(3, 3, 3, 3, num_hidden, 32, 0.0) for _ in range(gnn_layers)]
        )
    ...
    def forward(self, X, h_V, batch_id, chain_encoding, topk=30, virtual_frame_num=3):
        ...
        (
            ...
            X, edge_idx, batch_id, chain_encoding, virtual_frame_num, self.atom_embed
        )
        dV = self.node_embedding(dV)

        dE = self.edge_embedding(dE) + self.chain_embedding(
            (chain_encoding[edge_idx[0]] == chain_encoding[edge_idx[1]]).long()
        )
    ...
    def message_passing_R_mat(self, T, T_ts, h_E, edge_idx, batch_id):
        ...
        t_s = T._trans
        src_idx, dst_idx = edge_idx[0], edge_idx[1]
        R_s, t_s = self.equilibrate_transforms(
            R_s, t_s, R_ts, t_ts, logit_ts, src_idx, dst_idx, all_num_node
        )
        return R_s, t_s

    def equilibrate_transforms(
        self, R_i_init, t_i_init, R_ji, t_ji, logit_ji, src_idx, dst_idx, all_num_node
    ):
        R_i_pred, t_i_pred = self.compose_transforms(
            R_i_init[dst_idx], t_i_init[dst_idx], R_ji, t_ji
        )
        ...
        R_avg_unc = scatter_sum(
            R_i_pred * R_probs[:, None, None], src_idx, dim=0, dim_size=L
        )
    ...
    def compose_transforms(
        self, R_a: torch.Tensor, t_a: torch.Tensor, R_b: torch.Tensor, t_b: torch.Tensor
    ):
    ...
    def build_graph(self, X, batch_id, topk=30):
        X = X[:, 1, :]
        edge_idx = knn_graph(
            X, k=topk, batch=batch_id, loop=False, flow="target_to_source"
        )
...
class StructDecoder(nn.Module):
    def __init__(self, num_decoder_layers, hidden_dim, steps=5, version="CATH-AE"):
        ...
        self.decoders = nn.ModuleList(
            nn.ModuleList(
                [
                    GeneralE3GNN(1, hidden_dim, version=version)
    ...
    def infer_X(
        self, X, h_V_enc, batch_id, chain_encoding, dec_topk=30, virtual_frame_num=3
    ):
