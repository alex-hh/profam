...
from torch_scatter import scatter_min, scatter_softmax, scatter_sum
...
class GraphTransform:
    ...
    def dense2sparse_node(self, h_V, mask):
        ...
        node_mask_select = lambda x: torch.masked_select(x, mask.unsqueeze(-1)).reshape(
            -1, x.shape[-1]
        )
    ...
    def dense_cat_edge(self, h_E, h_B, num_node):
        ...
        for b in range(B):
            temp = torch.cat(
                [h_E[b, : num_node[b]], h_B[b, : num_node[b]]], dim=1
            )  # 扩充K维度
            pad = torch.zeros(
                L + dL - num_node[b], *temp.shape[1:], device=temp.device
            )  # padding L维度, 如果考虑old node-->new node的边,在此之前需要扩充L维度, 这里不更新global frame，因此直接padding了。
