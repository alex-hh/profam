...
class VQLayer(nn.Module):
    def __init__(
        self, num_embeddings, embedding_dim, vq_dim, ema_k, ema_lambda, ema_weight
    ):
    ...
    def EMAUpdate(self, h_flat, distances, k=5, gamma=0.995):
        ...
        h_feat = (
            h_flat[topk_ids.view(-1)].reshape(k, self.num_embeddings, -1).mean(dim=0)
        )

        update_mask = torch.abs(dist).min(dim=0)[0] > 0.1
        updated_embedding = self.embedding.weight.data * gamma + h_feat * (1 - gamma)
        self.embedding.weight.data = (
            self.embedding.weight.data * (~update_mask[:, None])
            + updated_embedding * update_mask[:, None]
        )

    def embed_id(self, vq_id):
        self.embedding.weight.data = (
            self.embedding.weight.data
            / self.embedding.weight.data.norm(dim=-1, keepdim=True)
        )
    ...
    def get_vq(self, h, attn_mask=None, mode="train", temperature=None):
        ...
        distances = (
            torch.sum(h_flat**2, dim=1, keepdim=True)
            + torch.sum(self.embedding.weight**2, dim=1)
            - 2 * torch.matmul(h_flat, self.embedding.weight.t())
        )

        if attn_mask is not None:
            distances = distances * attn_mask.view(-1, 1) + 10000000 * (
                1 - attn_mask.view(-1, 1)
            )
    ...
    def forward(self, h, attn_mask=None, mode="train", temperature=None):
        ...
        vq_loss = self.MSE(h.detach(), quantized) + 0.25 * self.MSE(
            h, quantized.detach()
        )
        if attn_mask is not None:
            vq_loss = (
                (vq_loss * attn_mask[:, :, None]).sum(dim=1)
                / attn_mask[:, :, None].sum(dim=1)
            ).mean()
        else:
...
class SoftVQLayer(nn.Module):
    ...
    def attention(self, H, C, temperature=1):
        distances = (
            torch.sum(H**2, dim=1, keepdim=True)
            + torch.sum(C**2, dim=1)
...
class SoftGVQLayer(nn.Module):
    ...
    def embed_id(self, vq_id):
        ...
        shift = 2 * torch.range(
            0, self.log2_num_embeddings - 1, device=vq_id.device, dtype=torch.long
        )
        vq_id = vq_id[:, :, :-1] + shift
    ...
    def get_vq(self, h, attn_mask, temperature, proj_inv=True):
        ...
        quantized[attn_mask == 1] = h_vq
        vq_code = torch.zeros(
            h.shape[:2] + (self.log2_num_embeddings,), device=h.device, dtype=torch.long
        )
    ...
    def forward(self, h, attn_mask=None, mode="train", temperature=1, proj_inv=True):
        ...
        self.embedding.weight.data = self.embedding.weight.data / (
            self.embedding.weight.data.norm(dim=-1, keepdim=True) + 1e-8
        )
        ...
        if mode == "train":
            ...
            shift = 2 * torch.range(
                0, self.log2_num_embeddings - 1, device=A.device, dtype=torch.long
            )
...
class SoftCVQLayer(nn.Module):
    def __init__(
        self, log2_num_embeddings, embedding_dim, vq_dim, condition_layer=6, sphere=True
    ):
    ...
    def binary2decimal(self, binary_vector):
        base = 2 ** torch.arange(
            binary_vector.size(-1) - 1, -1, -1, device=binary_vector.device
        )
    ...
    def forward(
        ...
        if vqshortcut and temperature > 0.02:
            ...
            replace = torch.zeros(
                h_in.shape[0], 1, device=h_in.device, dtype=h_in.dtype
            )
            replace[keep_idx] = 1
...
class HierCVQLayer(nn.Module):
    def __init__(
        ...
        self.embedding_mlp = nn.Sequential(
            *[
                self.build_condition_layer(
                    log2_num_embeddings, hidden_dim, condition_layer, vq_dim
                )
                for i in range(log2_num_embeddings)
            ]
        )
    ...
    def build_condition_layer(
        self, log2_num_embeddings, hidden_dim, condition_layer, vq_dim
    ):
        layers = [
            nn.Linear(log2_num_embeddings, hidden_dim),
            nn.BatchNorm1d(hidden_dim),
    ...
    def get_bvectors(self, num_embeddings):
        ...
        bool_vectors = (
            int_range[:, None] & (1 << torch.arange(num_embeddings - 1, -1, -1))
        ) > 0
    ...
    def embed_id(self, vq_id, level):
        ...
        bvectors = F.pad(
            bvectors, (self.log2_num_embeddings - level, 0, 0, 0), value=-1
        ).to(vq_id.device)
    ...
    def mask_code(self, code):
        ...
        index_tensor = torch.randint(
            0, self.log2_num_embeddings, (code.shape[0],), device=code.device
        )
    ...
    def forward(
        ...
        if mode == "train":
            ...
            for idx, level in enumerate(self.levels):
                quantized_, code, vq_loss_ = self.sample_code(
                    h, level, temperature, attn_mask
                )
                select = (idx / len(self.levels) < p) & (
                    p < (idx + 1) / len(self.levels)
                )
            ...
            quantized, code, vq_loss = self.sample_code(
                h, level, temperature, attn_mask
            )
...
class FSQ(nn.Module):
    ...
    def __init__(self, levels: list[int], embedding_dim, vq_dim, eps: float = 1e-3):
        ...
        self._implicit_codebook = self.indexes_to_codes(
            torch.arange(self.codebook_size, dtype=torch.int64)
        )
        self.proj = nn.Linear(embedding_dim, vq_dim)
        self.proj_inv = nn.Linear(vq_dim, embedding_dim)
    ...
    def bound(self, z: torch.Tensor) -> torch.Tensor:
        ...
        return torch.tanh(
            z + torch.tensor(shift, dtype=z.dtype, device=z.device)
        ) * torch.tensor(half_l, dtype=z.dtype, device=z.device) - torch.tensor(
            offset, dtype=z.dtype, device=z.device
        )
    ...
    def _scale_and_shift(self, zhat_normalized):
        ...
        return (
            zhat_normalized
            * torch.tensor(
                half_width, dtype=zhat_normalized.dtype, device=zhat_normalized.device
            )
        ) + torch.tensor(
            half_width, dtype=zhat_normalized.dtype, device=zhat_normalized.device
        )
    ...
    def _scale_and_shift_inverse(self, zhat):
        half_width = self._levels_np // 2
        return (
            zhat - torch.tensor(half_width, dtype=zhat.dtype, device=zhat.device)
        ) / torch.tensor(half_width, dtype=zhat.dtype, device=zhat.device)
    ...
    def codes_to_indexes(self, zhat: torch.Tensor) -> torch.Tensor:
        ...
        return (
            (zhat * torch.tensor(self._basis, dtype=zhat.dtype, device=zhat.device))
            .sum(dim=-1)
            .type(torch.int64)
        )  # 修改此处为 torch.int64
