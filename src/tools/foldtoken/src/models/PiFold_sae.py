...
class PiFold_Model(nn.Module):
    ...
    def forward(
        ...
        if frozen_vq:
            ...
            h_V = h_V[chain_encoding < 1000]
            h_V, vq_code, vq_loss = self.vq(
                h_V, temperature=temp, mode=mode, vqshortcut=vqshortcut, frozen=False
            )
    ...
    def sample(self, protein_init):
        ...
        chain_encoding = torch.cat(
            [
                torch.zeros_like(batch_id),
                torch.ones(3, device=batch_id.device).long() * 1001,
            ]
        )
        batch = {
            "X_true": X,
    ...
    def encoding(
        ...
        h_V = h_V[chain_encoding < 1000]
        h_V, vq_code, vq_loss = self.vq(
            h_V, temperature=temp, mode=mode, vqshortcut=vqshortcut
        )
