...
from src.chroma.layers.structure.diffusion import (
    DiffusionChainCov,
...
class PiFold_Model(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        self.vq = HierCVQLayer(
            args.vq_space, hidden_dim, 32, args.levels, args.condition_layer
        )

        self.encoder = StructureEncoder(
            3, 3, 3, 3, args.enc_layers, hidden_dim, dropout=0.0
        )
    ...
    def compute_loss(self, all_preds, X, batch_id):
        ...
        for pred_X in all_preds:
            ...
            all_results["batch_neighborhood_mse"] += (
                results["batch_neighborhood_mse"] / L
            )
            all_results["batch_distance_mse"] += results["batch_distance_mse"] / L
        pred_X, mask = GraphTransform.sparse2dense_node(pred_X, batch_id)

        loss = (
            all_results["batch_global_mse"]
            + all_results["batch_fragment_mse"]
            + all_results["batch_pair_mse"]
            + all_results["batch_neighborhood_mse"]
            + all_results["batch_distance_mse"]
        )
    ...
    def forward(
        ...
        h_V, vq_code, vq_loss = self.vq(
            h_V, temperature=temp, mode=mode, vqshortcut=vqshortcut, level=level
        )

        X_pred, all_preds = self.decoder_struct.infer_X(
            X_t,
            h_V,
            batch_id,
            chain_encoding,
    ...
    def sample(self, protein_init):
        ...
        X = X[0]
        edge_idx = knn_graph(
            X[:, 1], k=30, batch=batch_id, loop=True, flow="target_to_source"
        )
        ...
        (
            ...
            X, edge_idx, batch_id, chain_encoding, virtual_frame_num=3
        )
        X_t = torch.rand_like(X)
        temp = 1e-8
        mode = "eval"

        X_pred, all_preds, vq_loss, vq_count, vq_code = self(
            X,
            chain_encoding,
            edge_idx,
            batch_id,
    ...
    def decoding(self, h_V, chain_encoding, batch_id=None, returnX=False):
        ...
        X_pred, all_preds = self.decoder_struct.infer_X(
            X_t, h_V, batch_id, chain_encoding, 30, virtual_frame_num=virtual_frame_num
        )
