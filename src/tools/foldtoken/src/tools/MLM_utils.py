...
from openfold.data import data_transforms, protein, residue_constants
...
class CropPadData:
    ...
    def __call__(self, data):
        ...
        # Perform padding
        if angles.shape[0] < pad:
            angles = F.pad(angles, (0, 0, 0, pad - angles.shape[0]))
...
class Chain2AF2:
    ...
    def __call__(self, data):
        ...
        data = transform.Atom37(
            AF2Data.aatype,
            AF2Data.atom_positions,
            AF2Data.atom_mask,
            AF2Data.chain_index,
            AF2Data.residue_index,
            AF2Data.atom_mask,
            max_res_num=seqs.shape[0],
        )
    ...
    def __call__(self, x: torch.Tensor):
        ...
        * `x` is the batch of input token sequences.
        ...
        random_tokens = torch.randint(
            0, self.n_tokens, (len(random_token_idx[0]),), device=x.device
        )
...
class MLM_Angle:
    ...
    def __call__(self, x: torch.Tensor):
        ...
        * `x` is the batch of input token sequences.
        ...
        # A mask for tokens to be replaced with original tokens
        unchanged = full_mask & (
            torch.rand(shape, device=x.device) < self.no_change_prob
        )
        # A mask for tokens to be replaced with a random token
        random_token_mask = full_mask & (
            torch.rand(shape, device=x.device) < self.randomize_prob
        )
        # Indexes of tokens to be replaced with random tokens
        random_token_idx = torch.nonzero(random_token_mask, as_tuple=True)
        # Random tokens for each of the locations
        random_tokens = (
            torch.rand((len(random_token_idx[0]), 3), device=x.device, dtype=x.dtype)
            * 2
            * math.pi
            - math.pi
        )  # -math.pi, math.pi,
