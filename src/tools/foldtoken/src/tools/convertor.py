...
from openfold.data import protein, residue_constants
...
def get_dihedral(atoms1, atoms2, atoms3, atoms4):
    ...
    dalta_y = dot(
        torch.cross(n1, n2), v2
    )  # torch.cross(n1,n2)与v2共线, 但方向不一定相同, # n1,n2之间的sin值
...
class Converter:
    ...
    def coord2angle(self, x):
        ...
        X = torch.stack(
            [x_extend[:, 0:-3], x_extend[:, 1:-2], x_extend[:, 2:-1], x_extend[:, 3:]],
            dim=-2,
        )
    ...
    def angle2coord(self, bond_lengths, bond_angles, torsion_angles, x=None):
        ...
        if x is not None:
            ...
            x_init = (
                torch.rand(
                    B, 3, 3, device=bond_lengths.device, dtype=bond_lengths.dtype
                )
                * 10
            )
        ...
        all_pos = []
        for i in range(0, L):
            pred_x = place_dihedral(
                x_init[:, 0:1],
                x_init[:, 1:2],
                x_init[:, 2:3],
                bond_angles[:, i : i + 1],
                bond_lengths[:, i : i + 1],
                torsion_angles[:, i : i + 1],
            )
