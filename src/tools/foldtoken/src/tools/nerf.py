...
def rigid_transform_P2Q(P, Q, mask):
    ...
    centroid_P = torch.sum(P * mask[..., None], dim=-2, keepdims=True) / torch.sum(
        mask[..., None], dim=-2, keepdims=True
    )
    centroid_Q = torch.sum(Q * mask[..., None], dim=-2, keepdims=True) / torch.sum(
        mask[..., None], dim=-2, keepdims=True
    )
...
class TorchNERFBuilder_Oxygen(nn.Module):
    ...
    def cartesian_coords(self) -> np.ndarray:
        ...
        for i in range(self.phi.shape[1] - 1):
            phi, psi, omega, O_dihedral = (
                self.phi[:, 1 + i].view(-1, 1),
                self.psi[:, i].view(-1, 1),
                self.omega[:, i].view(-1, 1),
                self.O_dihedrals[:, i].view(-1, 1),
            )
            ...
            for bond, dih in zip(
                self.bond_lengths.keys(), [psi, omega, phi, O_dihedral]
            ):
                if bond in [("C", "N")]:
                    coords = self.place_dihedral(
                        retval[-4],
    ...
    def place_dihedral(
        ...
        d = torch.cat(
            [
                ...
                bond_length
                * torch.cos(torsion_angle)
                * torch.sin(bond_angle),  # 局部坐标系上y轴坐标
                bond_length
                * torch.sin(torsion_angle)
                * torch.sin(bond_angle),  # 局部坐标系上z轴坐标
            ],
            dim=-1,
        )
    ...
    def _get_bond_angles(self, source_struct):
        import biotite.structure as struc
        from biotite.structure import angle, chain_iter, distance, filter_amino_acids
        ...
        CA_C_N = angle(
            bb_coord[:, :-1, 1, :], bb_coord[:, :-1, 2, :], bb_coord[:, 1:, 0, :]
        )
        C_N_CA = angle(
            bb_coord[:, :-1, 2, :], bb_coord[:, 1:, 0, :], bb_coord[:, 1:, 1, :]
        )
        N_CA_C = angle(
            bb_coord[:, 1:, 0, :], bb_coord[:, 1:, 1, :], bb_coord[:, 1:, 2, :]
        )
        CA_C_O = angle(
            bb_coord[:, 1:, 1, :], bb_coord[:, 1:, 2, :], bb_coord[:, 1:, 3, :]
        )
    ...
    @classmethod
    def sv2pdb(self, out_fname: str, coords, unknown_mask=None) -> str:
        """
...
class TorchNERFBuilder(nn.Module):
    ...
    def set_values(
        ...
        if len(init_coords[0].shape) == 1:
            a = (
                torch.tensor(init_coords[0], device=self.device)
                .repeat(batch, 1)
                .float()
            )
            b = (
                torch.tensor(init_coords[1], device=self.device)
                .repeat(batch, 1)
                .float()
            )
            c = (
                torch.tensor(init_coords[2], device=self.device)
                .repeat(batch, 1)
                .float()
            )
    ...
    def place_dihedral(
        ...
        m = torch.stack(
            [bc, nbc, n], dim=1
        )  # [batch, number of vectors, vector values]
    ...
    def sv2pdb(self, out_fname: str, coords, unknown_mask=None, attn_mask=None) -> str:
        """
        Create a new chain using NERF to convert to cartesian coordinates. Returns
        the path to the newly create file if successful, empty string if fails.
        """
    ...
    def transform(self, pred_coords, pred_start, pred_end, fix_start, fix_end):
        ...
        pred_coords = (R @ pred_coords.permute(0, 2, 1)).permute(
            0, 2, 1
        ) + translation.mean(dim=1, keepdim=True)
        return pred_coords
...
def test_nerf():
    ...
    source = PDBFile.read(
        os.path.join(
            ...
            "/gaozhangyang/experiments/ProreinBinder/data/cath/dompdb/1a0aA00",
        )
    )
    ...
    pred_coords = builder.transform(
        pred_coords, pred_start, pred_end, fix_start, fix_end
    )
...
def test_gradient():
    ...
    for it in range(steps):
        ...
        omega = modulo_with_wrapped_range(
            omega, range_min=-torch.pi, range_max=torch.pi
        )
        C_N_CA = modulo_with_wrapped_range(
            C_N_CA, range_min=-torch.pi, range_max=torch.pi
        )
        N_CA_C = modulo_with_wrapped_range(
            N_CA_C, range_min=-torch.pi, range_max=torch.pi
        )
        CA_C_N = modulo_with_wrapped_range(
            CA_C_N, range_min=-torch.pi, range_max=torch.pi
        )
