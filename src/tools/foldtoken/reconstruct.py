...
def parse_args():
    ...
    parser.add_argument(
        "--config", default="./foldtoken/model_zoom/FT4/config.yaml", type=str
    )
    parser.add_argument(
        "--checkpoint", default="./foldtoken/model_zoom/FT4/ckpt.pth", type=str
    )
...
if __name__ == "__main__":
    ...
    for file_list in batch_list(file_names, 32):
        ...
        with torch.no_grad():
            ...
            list_protein_pred, list_vq_code = model.sample(
                batch=batch, level=args.level
            )
        ...
        for idx, title in enumerate(list_of_title):
            ...
            list_protein_pred[idx].to(
                output_prefix, mask_indices=None, seq=list_vq_code[idx].tolist()
            )
