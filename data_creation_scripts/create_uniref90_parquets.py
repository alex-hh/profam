...
def create_parquet_file(data, output_path):
    ...
    pq.write_table(table, output_path, compression='snappy', index=False)
                ...
                if current_header is not None:
                    ...
                    if sequence_count % rows_per_file == 0:
                        ...
                        output_file = os.path.join(
                            output_dir, 
                            f'uniref90_batch_{batch_index:03d}_part_{parquet_index:03d}.parquet'
                        )
                        create_parquet_file(data, output_file)
                        print(f"Created parquet file: {output_file} with {len(sequences)} sequences")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fasta_file', type=str, required=True, help="Path to the UniRef90 FASTA file")
