...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_index", type=int, required=True, help="Task index to process")
    args = parser.parse_args()

    # Find which dataset this task belongs to
    target_dataset = None
    for dataset in datasets_to_convert:
        start, end = dataset["task_range"]
        if start <= args.task_index <= end:
            target_dataset = dataset
            break
    
    if target_dataset is None:
        raise ValueError(f"Task index {args.task_index} is not within any dataset's task range")

    print(f"Processing task {args.task_index} for dataset {target_dataset['name']}")
    
    # Calculate relative task index within this dataset's range
    relative_task_index = args.task_index - target_dataset["task_range"][0]
    total_tasks = target_dataset["task_range"][1] - target_dataset["task_range"][0] + 1
    
    # Collect all parquet files across all subdirectories
    all_parquet_files = []
    for subdir in ["train_filtered", "val_filtered", "test_filtered", "train", "val", "test"]:
        parquet_dir = os.path.join(target_dataset['parquet_dir'], subdir)
        if os.path.exists(parquet_dir):
            all_parquet_files.extend(glob.glob(os.path.join(parquet_dir, "*.parquet")))
    
    # Sort files to ensure consistent ordering
    all_parquet_files.sort()
    
    # Calculate which files this task should process
    files_per_task = len(all_parquet_files) / total_tasks
    start_idx = int(relative_task_index * files_per_task)
    if args.task_index == target_dataset["task_range"][1]:
        end_idx = len(all_parquet_files)
    else:
        end_idx = int((relative_task_index + 1) * files_per_task)
    task_files = all_parquet_files[start_idx:end_idx]
    
    print(f"Task {args.task_index} will process {len(task_files)} files")
    
    # Process the assigned files
    for parquet_file in task_files:
        subdir = os.path.basename(os.path.dirname(parquet_file))
        if "output_dir" in target_dataset:
            output_dir = os.path.join(target_dataset["output_dir"], subdir)
        elif "parquets" in parquet_file:
            output_dir = os.path.join(os.path.dirname(parquet_file).replace("parquets", "text"))
        else:
            raise ValueError(f"No output directory specified for {target_dataset['name']}")
