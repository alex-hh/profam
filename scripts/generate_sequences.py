...
"""
Script to sample sequences from a ProFam checkpoint, conditioned on a FASTA/MSA.

Backwards-compatibility wrapper over the ``profam`` package. The CLI matches
the previous ``scripts/generate_sequences.py`` contract; model loading delegates
to ``profam.checkpoint.load_model``.
"""
