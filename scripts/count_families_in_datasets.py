...
def main():
    ...
    foldseek_file = os.path.join(
        base_data_dir, "afdb/1-AFDBClusters-entryId_repId_taxId.tsv"
    )
    df_foldseek = pd.read_csv(
        foldseek_file, sep="\t", header=None, names=["id", "clust", "tax"]
    )
