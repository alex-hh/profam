    ...
    for line in file:
        if line.startswith('>'):
            if current_id is not None:
                ...
                cursor.execute('SELECT sequence_id FROM sequences WHERE sequence_id = ?', (current_id,))
                # if the sequence is not in the database, add it
                if not cursor.fetchone():
                    cursor.execute('INSERT INTO sequences (sequence_id, sequence) VALUES (?, ?)', (current_id, current_sequence))
                    additional_seqs += 1
        ...
        if line_counter % 10000 == 0:
            print(f"Processed {line_counter} lines, added {additional_seqs} additional sequences")
    if current_id is not None:
        # check if the sequence is already in the database
        cursor.execute('SELECT sequence_id FROM sequences WHERE sequence_id = ?', (current_id,))
        # if the sequence is not in the database, add it
        if not cursor.fetchone():
            cursor.execute('INSERT INTO sequences (sequence_id, sequence) VALUES (?, ?)',
