...
def evaluate_generated_sequences_profam_on_ec_multi_sequence():
    ...
    for i, generated_fasta in enumerate(generated_fasta_paths):
        ...
        if ec_num in completed_ec_nums:
            ...
            continue
        prompt_fasta = f"../data/ec/ec_validation_dataset_clustered_c70_pid_30/alignments/{ec_num}_aln.filtered.fasta"
        if not os.path.exists(prompt_fasta):
            print(f"Prompt FASTA not found for {generated_fasta}")
            continue
        results = sequence_only_evaluation(prompt_fasta, generated_fasta, generate_logos=False)
...
if __name__ == "__main__":
    ...
    generated_pdb_pattern = "../sampling_results/colabfold_outputs/foldseek_combined_val_test_poet_exact_prompts_no_ensemble_2025_10_01/*/*.pdb"
    ...
    for i, generated_pdb in enumerate(generated_pdbs):
        ...
        if len(df) > 0 and generated_pdb in df["generated_pdb"].values:
            ...
            continue
        prompt_fasta_path = glob.glob(f"../data/val_test_v2_fastas/foldseek/combined_val_test/{generated_id}*.fasta")[0]
        prompt_pdb_paths = get_pdb_paths_from_fasta_path(prompt_fasta_path, gt_pdbs)
        print(f"number of prompt pdb paths: {len(prompt_pdb_paths)}")
        try:
