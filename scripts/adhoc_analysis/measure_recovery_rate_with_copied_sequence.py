...
def evaluate_sequence_recovery_rate(sequences, eval_sequence):
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.fasta', delete=False) as temp_in, \
             tempfile.NamedTemporaryFile(mode='w', suffix='.fasta', delete=False) as temp_out:
...
for i, parquet_file in enumerate(parquets):
    ...
    for _, row in df.iterrows():
        ...
        recovery_rate = evaluate_sequence_recovery_rate(ranked_sequences, evaluation_sequence)
