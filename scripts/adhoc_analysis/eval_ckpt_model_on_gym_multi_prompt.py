...
def _generate_context_variants(
    ...
    """Create a set of context-truncated variants that *never* exceed *context_tokens_limit*.

    For every possible number of context sequences *n* (including *n = 0*) that
    fits within the token budget we create *subsamples_per_n* variants.  The
    variants for *n > 0* are generated by randomly selecting a contiguous
    **block** of *n* sequences from the full MSA until the resulting token
    count is ≤ *context_tokens_limit* (falling back to the first *n*
    sequences if no such block is found within a small number of attempts).
    """

    input_ids = batch["input_ids"]
    if input_ids is None:
        raise ValueError("input_ids must not be None for context subsampling")

    # Locate the end index (inclusive) of every context sequence
    seq_ends = (input_ids[0] == sep_tok_id).nonzero(as_tuple=True)[0].cpu()
    total_seqs = len(seq_ends)

    # Pre-compute the token count for the *prefix* containing the first k sequences
    prefix_token_counts = seq_ends + 1  # +1 because indices are 0-based

    # Maximum number of sequences that can fit under the token budget if we take
    # them contiguously from the *start* of the document
    max_n_under_limit = int((prefix_token_counts <= context_tokens_limit).sum())
    ...
    var_batch = _clone_batch(batch)
    var_batch["input_ids"] = None
    if "residue_index" in var_batch:
        var_batch["residue_index"] = None  # type: ignore[assignment]
    variants.append(
        (
            var_batch,
            {
                "variant_type": "no_context",
                "n_seqs": 0,
                "n_tokens": 0,
                "replicate": 0,
            },
    ...
    # -----------------------------------------------------------
    # 1 ≤ n ≤ max_n_under_limit
    # -----------------------------------------------------------
    rng = random.Random()
    for n in range(1, max_n_under_limit + 1):
        for rep in range(subsamples_per_n):
            # Try a few times to pick a random contiguous *block* of n sequences
            # that fits under the token limit; fall back to the first n sequences
            for _ in range(10):
                start_seq = rng.randint(0, total_seqs - n)
                start_tok = seq_ends[start_seq - 1] + 1 if start_seq > 0 else 0
                end_tok = seq_ends[start_seq + n - 1].item()
                token_count = end_tok - start_tok + 1
                if token_count <= context_tokens_limit:
                    break
            else:
                # Fallback: use the first n sequences (guaranteed to fit)
                start_seq = 0
                start_tok = 0
                end_tok = seq_ends[n - 1].item()
                token_count = end_tok + 1

            var_batch = _clone_batch(batch)
            var_batch["input_ids"] = var_batch["input_ids"][..., start_tok : end_tok + 1].clone()
            if "residue_index" in var_batch and var_batch["residue_index"] is not None:
                var_batch["residue_index"] = var_batch["residue_index"][..., start_tok : end_tok + 1].clone()

            variants.append(
                (
                    var_batch,
                    {
                        "variant_type": "subsample",
                        "n_seqs": n,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--initial_max_context_tokens",
        type=int,
        default=128_000,
        help="Maximum number of context tokens to request from the dataloader (full MSA).",
    )
    # ------------------------------------------------------------------
    ...
    parser.add_argument(
        ...
        type=int,
        default=7_500,
        help="Maximum number of *context* tokens that may be fed to the model on any single forward pass.  The evaluation code will automatically subsample sequences so that no generated batch exceeds this limit.",
    )
    ...
    dataloader = build_protein_gym_dataloader(
        ...
        max_context_tokens=args.initial_max_context_tokens,
    ...
    csv_headers += ["ensemble_log_likelihood", "ensemble_spearman"]
    results = []
    for batch_idx, batch in enumerate(dataloader):
        ...
        # Generate context-truncated variants respecting the token budget
        variants = _generate_context_variants(
            batch_for_model,
            model.tokenizer.sep_token_id,
            context_tokens_limit=args.context_tokens_limit,
        )

        dms_scores_np = batch_for_model["DMS_scores"][0].cpu().numpy()
        variant_lls = []
        variant_spearman = []
        variant_meta = []
        for var_batch, meta in variants:
            L = var_batch["completion_ids"].shape[-1]
            L_prompt = var_batch["input_ids"].shape[-1]
            lls = model.score_seqs(
                var_batch["input_ids"],
                var_batch["completion_ids"],
                input_residue_index=var_batch.get("residue_index", None),
                completion_residue_index=var_batch.get("completion_residue_index", None),
                use_cache=model.use_kv_cache_for_scoring,
                batch_size=1,
            )
            variant_lls.append(lls)
            sp = _compute_spearman(lls, dms_scores_np)
            variant_spearman.append(sp)
            meta["spearman"] = sp
            variant_meta.append(meta)

        lls_array = np.stack(variant_lls, axis=0)
        # save raw likelihoods and meta for offline ensembling
        np.savez_compressed(
            os.path.join(args.output_dir, f"batch_{batch_idx}_likelihoods.npz"),
            lls=lls_array,
            meta=np.array(variant_meta, dtype=object),
            dms_scores=dms_scores_np,
        )

        # Simple ensemble by mean of likelihoods
        mean_lls = lls_array.mean(axis=0)
        ensemble_spearman = _compute_spearman(mean_lls, dms_scores_np)
        ...
        row_data = {
            ...
            "ensemble_log_likelihood": ensemble_log_ll,
            "ensemble_spearman": ensemble_spearman,
            "log_likelihood": ensemble_log_ll,
            "spearman_correlation": ensemble_spearman,
        }
