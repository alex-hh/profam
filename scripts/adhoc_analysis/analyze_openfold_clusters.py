...
def analyze_sequences(sequences: np.ndarray) -> dict:
    ...
    if num_seq_pairs > 0:
        ...
        seq_pairs = random.sample([(i, j) 
                                 for i in range(len(sequences)) 
                                 for j in range(i+1, len(sequences))], 
                                num_seq_pairs)
        identities = [compute_seq_identity(sequences[i], sequences[j]) 
                     for i, j in seq_pairs]
...
def main():
    # Parameters
    input_pattern = "../data/openfold/uniclust30_filtered_parquet_fragments_debugging/*.parquet"
    ...
    print(f"Sequence statistics saved to {os.path.join(output_dir, 'sequence_statistics.csv')}")
