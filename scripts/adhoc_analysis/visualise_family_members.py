...
def main():
    ...
    accession_sequence_map = get_sequences_for_accessions(
        random_row, sampled_accessions
    )
    ...
    output_image = (
        f'{out_dir}/{"_".join([a.replace("/", "_") for a in sampled_accessions])}.png'
    )
...
def get_sequences_for_accessions(random_row, sampled_accessions):
    ...
    accession_sequence_subset = {
        acc: accession_sequence_map[acc] for acc in sampled_accessions
    }
