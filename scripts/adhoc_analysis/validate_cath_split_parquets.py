...
def validate_parquets():
    ...
    datasets = {
        ...
        5: {
            'name': 'foldseek_af50',
            'parent_dir': '../data/foldseek_af50',
            'split_dir': '../data/foldseek_af50/train_val_test_split'
        },
        6: {
            'name': 'foldseek_af50_representatives',
            'parent_dir': '../data/foldseek_af50_representatives',
            'split_dir': '../data/foldseek_af50_representatives/train_val_test_split'
        },
        7: {
            'name': 'foldseek_af50_struct',
            'parent_dir': '../data/foldseek_af50_struct',
            'split_dir': '../data/foldseek_af50_struct/train_val_test_split'
        },
        8: {
            'name': 'foldseek_representatives',
            'parent_dir': '../data/foldseek_representatives',
            'split_dir': '../data/foldseek_representatives/train_val_test_split'
        },
        9: {
            'name': 'foldseek_struct',
            'parent_dir': '../data/foldseek_struct',
            'split_dir': '../data/foldseek_struct/train_val_test_split'
        }
    }
    ...
    for dataset_id, dataset_info in datasets.items():
        ...
        logging.info(f"Found {len(parent_parquet_files)} parent parquet files in directory {parent_dir}")
        ...
        for split in splits:
            ...
            logging.info(f"Processing '{split}' split. Found {len(split_parquet_files)} parquet files in directory {split_dir_split}")
            ...
            for parquet_file in split_parquet_files:
                ...
                for idx, row in df.iterrows():
                    ...
                    if accessions is None or sequences is None:
                        ...
                        parquet_file_stats['rows_with_invalid_accessions_or_sequences'] += 1
                        continue
                    ...
                    if len(accessions) == 0 or len(sequences) == 0:
                        ...
                        parquet_file_stats['rows_with_empty_accessions_or_sequences'] += 1
                    ...
                    if seq_lengths:
                        ...
                        if parquet_file_stats['min_sequence_length'] is None or min_length < parquet_file_stats['min_sequence_length']:
                            parquet_file_stats['min_sequence_length'] = min_length
                        if dataset_stats['min_sequence_length'] is None or min_length < dataset_stats['min_sequence_length']:
            ...
            dataset_stats['rows_with_invalid_accessions_or_sequences'] += invalid_accessions_or_sequences
            dataset_stats['rows_with_empty_accessions_or_sequences'] += empty_accessions_or_sequences
            dataset_stats['count_accessions_len_less_than_2'] += accessions_len_less_than_2
            if dataset_stats['min_sequence_length'] is None or (parquet_file_stats['min_sequence_length'] is not None and parquet_file_stats['min_sequence_length'] < dataset_stats['min_sequence_length']):
                dataset_stats['min_sequence_length'] = parquet_file_stats['min_sequence_length']

            logging.info(f"Completed processing '{split}' split for dataset '{dataset_name}'. Total rows: {total_split_rows_split}")

        # Total split rows
        total_split_rows = dataset_stats['total_train_rows'] + dataset_stats['total_val_rows'] + dataset_stats['total_test_rows']
        dataset_stats['total_split_rows'] = total_split_rows

        # Check total rows
        if total_parent_rows != total_split_rows:
            logging.warning(f"Total rows in splits ({total_split_rows}) do not match parent total rows ({total_parent_rows}) for dataset '{dataset_name}'")
    ...
    logging.info("Converting dataset results to DataFrame and saving to 'validation_results_by_dataset.csv'")
    dataset_results_df = pd.DataFrame(dataset_results)
    dataset_results_df.to_csv('validation_results_by_dataset.csv', index=False)

    logging.info("Converting parquet file results to DataFrame and saving to 'validation_results_by_parquet_file.csv'")
    parquet_file_results_df = pd.DataFrame(parquet_file_results)
    parquet_file_results_df.to_csv('validation_results_by_parquet_file.csv', index=False)
