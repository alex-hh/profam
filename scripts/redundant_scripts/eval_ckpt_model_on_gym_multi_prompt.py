...
def _generate_context_variants(
    ...
    """Create a set of context-truncated variants that *never* exceed *max_tokens*.
    ...
    **block** of *n* sequences from the full MSA until the resulting token
    count is ≤ *max_tokens* (falling back to the first *n*
    ...
    max_n_under_limit = int((prefix_token_counts <= max_tokens).sum())
